...
def load_model():
    ...
    def safe_load_sentence_transformer(model_path):
        try:
            return SentenceTransformer(model_path)
        except Exception as e:
            logger.error(f"Failed to load sentence transformer {model_path}: {e}")
            raise

    try:
        embedding_model_positive = safe_load_sentence_transformer(positive_embedding_model_path)
        embedding_model_negative = safe_load_sentence_transformer(negative_embedding_model_path)
    except Exception as e:
